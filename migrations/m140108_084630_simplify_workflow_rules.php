...
class m140108_084630_simplify_workflow_rules extends CDbMigration
{
	...
	{
		if (count($this->dbConnection->createCommand()->select("*")->from("ophciexamination_workflow_rule")->where("subspecialty_id is null and episode_status_id is null")->queryAll()) >1) {
			throw new Exception("More than one root node found");
		}
		...
		$root_node = $this->dbConnection->createCommand()->select("*")->from("ophciexamination_workflow_rule")->where("subspecialty_id is null and episode_status_id is null")->queryRow();

		$this->update('ophciexamination_workflow_rule',array('clause' => 'subspecialty_id'),"id = {$root_node['id']}");

		foreach ($this->dbConnection->createCommand()->select("*")->from("ophciexamination_workflow_rule")->where("subspecialty_id is not null or episode_status_id is not null")->order("id asc")->queryAll() as $rule) {
			$clause = ($rule['episode_status_id'] === null && $this->dbConnection->createCommand()->select("*")->from("ophciexamination_workflow_rule")->where("subspecialty_id = :subspecialty_id and episode_status_id is not null",array(":subspecialty_id" => $rule['subspecialty_id']))->queryRow()) ? 'status_id' : null;

			if ($rule['episode_status_id'] === null) {
				$parent_id = $root_node['id'];
				$value = $rule['subspecialty_id'];
			} else {
				$parent = $this->dbConnection->createCommand()->select("*")->from("ophciexamination_workflow_rule")->where("subspecialty_id = :subspecialty_id and episode_status_id is null",array(":subspecialty_id" => $rule['subspecialty_id']))->queryRow();
				$parent_id = $parent['id'];
